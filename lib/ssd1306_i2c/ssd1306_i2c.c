...
inline static int ssd1306_write_raw(ssd1306_inst_t *ssd_dev, const uint8_t *src, size_t len) {
    int ret = i2c_write_blocking(ssd_dev->i2c_dev, ssd_dev->addr, src, len, false);
    switch(ret) {
...
void ssd1306_init(ssd1306_inst_t *ssd_dev) {
    ...
    uint8_t init_seq[] = {
        ...
        SSD1306_CMD_SET_MUX_RATIO,          ssd_dev->height - 1,
        ...
        SSD1306_CMD_SET_COM_PINS,           ssd_dev->width > 2 * ssd_dev->height ? SSD1306_COM_PINS_SEQUENTIAL : SSD1306_COM_PINS_ALTERNATIVE,

        /* display */
        SSD1306_CMD_SET_CONTRAST,           0xFF,
        SSD1306_CMD_SET_PRECHARGE,          SSD1306_PRECHARGE_PHASE2_15 | SSD1306_PRECHARGE_PHASE1_1,
        SSD1306_CMD_SET_VCOMH_DESELECT,     SSD1306_VCOMH_083_VCC,
        SSD1306_CMD_DISPLAY_ALL_ON_RESUME,
        SSD1306_CMD_NORMAL_DISPLAY,

        /* addressing */
        SSD1306_CMD_SET_MEM_ADDR_MODE,      SSD1306_MEM_ADDR_HORIZONTAL,
...
void ssd1306_update(ssd1306_inst_t *ssd_dev) {
    uint8_t cmd_seq[] = {
        SSD1306_CMD_SET_COL_ADDR, 0, ssd_dev->width-1,
...
void ssd1306_draw_square(ssd1306_inst_t *ssd_dev, uint32_t x, uint32_t y, uint32_t width, uint32_t height) {
    for(uint32_t i=0; i<width; ++i)
        for(uint32_t j=0; j<height; ++j)
            ssd1306_draw_pixel(ssd_dev, x+i, y+j);
}

void ssd1306_draw_char_with_font(ssd1306_inst_t *ssd_dev, uint32_t x, uint32_t y, uint32_t scale, const uint8_t *font, char c) {
    ...
    for(uint8_t w=0; w<font[1]; ++w) { // width
        ...
        for(uint32_t lp=0; lp<parts_per_line; ++lp) {
            ...
            for(int8_t j=0; j<8; ++j, line>>=1) {
                if(line & 1)
                    ssd1306_draw_square(ssd_dev, x+w*scale, y+((lp<<3)+j)*scale, scale, scale);
            }
...
void ssd1306_draw_string_with_font(ssd1306_inst_t *ssd_dev, uint32_t x, uint32_t y, uint32_t scale, const uint8_t *font, const char *s) {
    for(int32_t x_n=x; *s; x_n+=(font[1]+font[2])*scale) {
        ssd1306_draw_char_with_font(ssd_dev, x_n, y, scale, font, *(s++));
    }
}

void ssd1306_draw_char(ssd1306_inst_t *ssd_dev, uint32_t x, uint32_t y, uint32_t scale, char c) {
    ssd1306_draw_char_with_font(ssd_dev, x, y, scale, font_8x5, c);
}

void ssd1306_draw_string(ssd1306_inst_t *ssd_dev, uint32_t x, uint32_t y, uint32_t scale, const char *s) {
